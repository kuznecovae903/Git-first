...
Удаленные репозитории на GitHub используются для организации совместной работы нескольких исполнителей с одним репозиторием на портале GitHub. Для работы на портале GitHub следует зарегистрироваться (создать аккаунт) следуя подсказкам портала.

### Cоздание локального репозитория

Необходимо для работы инициализировать, при необходимости воспользовавшись подсказками GitHub, локальный репозиторий (см.раздел Инициализация репозитория).

### Cоздание удаленного репозитория на GitHub.

Создание репозитория на GitHub осуществляется с помощью опции `New repository` на личной странице пользователя. При создании указывается название, присваивается признак (личный/публичный), также указывается ряд параметров, позволяющих в т.ч. создать файлы *Readme* (описание содержания репозитория), "*.ignore*".
Далее использовать команду `Creat repository`

### Привязка удаленного и локального репозитория.
...
Для того, чтобы скопировать (склонировать) удаленный репозиторий на локальный компьютер для работы следует использовать команду 
```
git clone <имя репозитория> <url-адрес репозитория>
```
При последующей раоте с "чужим" репозиторием следует создать новую свою ветку, в которую вносить дополнения/изменения.

### Скачивание и одновременное слияние информации из удаленного репозитория с локальным репозиторием.

Для скачивания всей инфомрации из текущего удаленного репозитория в локальный следует использовать команду 
```
git pull
```
Внимание: эта команда одноверменно автоматически "сливает" информацию локального и удаленного репозитория (делает merge).

При скачивании инфомрации из чужого удаленного репозитория следует:
